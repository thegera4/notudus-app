...
describe('components/notes/ListNoteItem', () => {
  beforeEach(() => {
    ;(animateListItem as jest.Mock).mockClear()
  })

  it('renders the note title, description and last modified date', () => {
    const tree = renderTree(<ListNoteItem note={note} onPress={jest.fn()} />)
    expect(hasText(tree, 'My Note')).toBe(true)
    expect(hasText(tree, 'Some content here')).toBe(true)
    expect(hasText(tree, 'Last modified: 2026-01-02')).toBe(true)
  ...
  it('shows the Private label for locked notes', () => {
    const tree = renderTree(
      <ListNoteItem note={{ ...note, locked: 1 }} onPress={jest.fn()} />,
    )
  ...
  it('calls onPress with the note when pressed', () => {
    ...
    const tree = renderTree(<ListNoteItem note={note} onPress={onPress} />)
